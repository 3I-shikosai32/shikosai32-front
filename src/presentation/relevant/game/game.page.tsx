...
export const PresidentGamePage: FC = () => {
  ...
  return (
    <GamePlayground {...props} className="w-screen grow text-game-president-g2 gradient-game-president">
      ...
      <GamePlaygroundSeparator />
      <span className="text-xs">
        3が一番弱く、続いて4、5 … K、A、2というように強くなっている。
        <br />
        ジョーカーは足りないカードの代わりになり、一枚で使うと最強になる。
        <br />
        始めに、スペードの3を持っているプレイヤーが好きなカードを出す。
        <br />
        順番に一人ずつ手札から選んでカードを出していくが、場に出ているカードよりも強いカードを出さなくてはいけない。
        <br />
        また、カードを最初に出した人が同じカードを2枚や3枚など複数で出した場合は次の人もそれより強いカードを同じ枚数で出さなくてはいけない。
        <br />
        手持ちのカードを出せない時や、出したくない時はパスをすることができる。
        <br />
        全員がパスをした場合は、場に出ているカードを流して、前の時に最後に出したプレイヤーが好きなカードを出して続ける。
        <br />
        手札がなくなったプレイヤーの勝ち。
        <br />
      </span>
